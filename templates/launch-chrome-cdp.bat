...
rem  After it opens: log in to chatgpt.com in that Chrome window and keep
rem  it open. No need to restart Claude Code: the Playwright MCP attaches
rem  on first use (if the first call after relaunching Chrome fails, retry once).
...
echo.
echo  Next: log in to chatgpt.com in that Chrome window and keep it open.
echo        No need to restart Claude Code - the MCP attaches on first use.
