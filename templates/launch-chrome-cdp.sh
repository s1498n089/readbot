...
#  之後在開啟的視窗手動登入 chatgpt.com、保持開著即可；不必重啟 Claude Code——
#  MCP 會在第一次用到瀏覽器工具時自動接管（剛重開 Chrome 後首次呼叫若失敗，重試一次即可）。
...
echo
echo " Next: 在那個視窗登入 chatgpt.com、保持開著即可；不必重啟 Claude Code，MCP 用到時會自動接管。"
