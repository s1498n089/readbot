...
### ⚠️ 工具名有「範疇解析」：dev 與安裝版前綴不同（會咬到權限預核准）
**同一台 MCP server，工具的「全名」會因「怎麼載入」而不同**——這是實戰咬過的點，因為權限 `allow`／`ask` 的字串要**一字不差**對上工具真名：

| 載入方式 | 工具全名 | 例（playwright server 的 `browser_click`） |
|---|---|---|
| **開發**：`claude --plugin-dir .`／在 repo 內，吃**專案級** `.mcp.json` | `mcp__<server>__<tool>` | `mcp__playwright__browser_click` |
| **安裝**：`/plugin install`，吃 **plugin 自帶**的 `.mcp.json` | `mcp__plugin_<plugin>_<server>__<tool>` | `mcp__plugin_readbot_playwright__browser_click` |

被當「plugin」載入時，名字會多包一層 `plugin_<plugin名>_` 前綴（這就是「範疇解析」）。所以：
- **開發**：你 checked-in 的 `.claude/settings.json` 寫 `mcp__playwright__*` 就對了。
- **setup skill 幫安裝者預核准**時：要寫 `mcp__plugin_<plugin>_playwright`（例 `mcp__plugin_readbot_playwright`）——授整台 server，一行涵蓋所有 `browser_` 工具。
- 兩者**不能互換**：開發的名字放不進安裝情境，反之亦然。不確定真名 → 用 **`/mcp`**（或 ToolSearch）看實際工具名再決定。

> 註：`--plugin-dir` 只把 **skills** 變 `/<plugin>:*`；它**不會**把 MCP 改成 plugin 前綴。你在 repo 開發時若 enable 了**專案級** `.mcp.json`（`enabledMcpjsonServers`），那台 MCP 的工具仍是 `mcp__<server>__*`。`mcp__plugin_<plugin>_...` 只在真的 `/plugin install` 後才出現。
...
7. **MCP 工具名有「範疇解析」**：dev（吃專案級 `.mcp.json`）是 `mcp__<server>__*`、安裝版（plugin 自帶）是 `mcp__plugin_<plugin>_<server>__*`。權限預核准要對齊**當下載入方式的真名**，別把兩者搞混（§4.5）。
...
- [ ] MCP 權限預核准：對齊**當下載入方式**的工具名（dev `mcp__<server>__*` vs 安裝 `mcp__plugin_<plugin>_<server>__*`，§4.5）
