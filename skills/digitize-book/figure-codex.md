...
- **驗收錨點是產物、不是退出碼**：codex（尤其 `run_in_background` 並行）可能退出碼 0 卻沒生圖（例如 codex 不在 PATH → 只印一行 command not found、圖一張沒生）。**每張都要確認輸出圖真的生出來、尺寸合理**，別憑退出碼。
...
- **文字對照（防圖文不一致）**：把**本章本文已定稿的術語譯名**整理一份、每張指令都帶上——同一個詞在圖裡要跟本文用同一個譯名（否則讀者會以為是兩個東西）；並指明**哪些不可翻**（路徑、識別碼、產品名、資料值）。
