...
- **流程＝接在 §5A 裁切後**，每張圖「**先裁切 → 再生譯圖**」：把剛裁好的 `figX-Y.png` 丟 ChatGPT，要求「**把圖內文字＋圖說翻成目標語言，其餘（圖形／座標軸／數據／曲線／數學符號／排版／配色）維持原樣**，重新輸出整張圖」。**預設做法＝請 GPT 用影像生成「重新繪製整張圖」（image-gen），這是優先選擇**——對結構／示意圖（流程圖、金字塔、漏斗、節點圖、方框圖…）最乾淨、文字換得徹底、不會有殘字問題（GPT 常自己就走這條）。**只有少數圖種要改走 PIL 疊字**（照片／勾勾／精確 code／數據曲線——見下方💡）。**建議範本**（`[…]` 換成該圖實際內容；照 §4a 用語；數學符號與變數、產品名不翻）：
   ...
   - **image-gen 也要「明講」——別靠它自己會走**：實測不寫明時，GPT 對純結構流程圖常自作主張改用 PIL 疊字、把版面蓋壞（`output_text` 標籤被白塊整塊蓋掉、方框線被蓋斷、右緣殘留原英文字尾；ch2 fig2-9／fig2-11 都中），補一句「改用影像生成重繪、別再疊字」重下一次才乾淨。所以範本已把「用影像生成重繪、不要疊字」寫死進去——和下方 💡「走 PIL 要明講並講理由」兩邊對稱。
...
- **🔍 文字專項驗收（譯圖最重要的一關，每張都做）**：譯圖的價值就是「文字翻對」，所以要**專門針對文字驗一遍**——不是整張看順眼就過。步驟：
  ...
  - 對不上 → 重生（指令點名要它修哪個字）；**同一張最多 2 次**。**2 次還錯先看「錯多大」再決定收尾、別一律整張作廢**：
    ...
    - **只有一兩個字錯、其餘全對**（版面／配色／其他文字都對）→ **本機 PIL 只補那一兩個字**是合理的最後收尾（image-gen 兩次都救不回才用、非常態）：定位到錯字位置 → 白色蓋掉 → 用**接近書體的字型**重寫正確的字（原文是斜體就仿斜體）、其餘像素完全不動。例：ch2 fig2-14 圖說「迭代」被 image-gen 寫成「送代」、點名重生 2 次都改不掉，但整張只錯這一個字 → 本機補那個「迭」字收尾，勝過整張作廢。
