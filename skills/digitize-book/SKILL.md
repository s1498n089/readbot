...
把頁面插圖截出來。標準流程＝每張圖「**先裁切（成 `figX-Y.png`）→ §5B 回讀驗收 → task=translate 再生譯圖（成 `figX-Y.<語言碼>.png`）**」；原裁切圖一律保留、看板**預設顯示原圖**。裁切要含**整張圖 ＋ 整段圖說**（圖說可能在圖下方、也可能在側邊排成窄欄——先看清在哪一側）。**給做法檔的輸入＝足以定位那張圖的畫面**：能給整頁頁圖就整頁；一頁多圖／版面很雜時**怎麼鎖定正確那張、見你這條路的做法檔**（別把「先切一塊」當每條路都要的步驟）。

**合格標準（4 條）**：
- ① **外框邊線完整**（圖本體＋圖外那圈細外框線四邊都不切，尤其曲線**峰頂**）。
- ② **外框線外側再留白邊**（別讓外框線貼在圖片邊緣、四周有呼吸空間，約 20~30px——切到框線一眼就看得出來、很扣分）。
- ③ **留白不含內文**（沒有正文段落、頁眉頁碼、前一張圖）。
- ④ **含整段圖說**（「图 X-Y …」/「Figure X-Y …」）——**不論在下方或側邊窄欄、整段都要**；抓 bbox 取「圖形墨跡 ∪ 圖說墨跡」的**聯集**，別只往下抓。
...
### 5B. 回讀驗收 ＋ 插回原文
- **逐張回讀驗收**（強制）：每張都自己 Read 回讀，注意力**放在四邊**對照 ①②③④（方框有沒有被切？峰頂在不在？留白有沒有沾到正文／前一張圖？圖說在哪側、整段有沒有含全？）。不符就重來——重跑該圖的裁切（見你這條路的做法檔）。⚠️ **別用「全部乾淨」一句帶過**，每張都要實際看過（圖多可派 subagent 複驗）。**疊字／本機補字這種「程式性改原圖」的，驗收也可程式性**——比對「只有該改處有差異」比目視硬；重畫的只能目視。
- **把圖插回原文出現的位置**（夾在對應段落之間、不是堆在章末）：插一個 **markdown cell**、**內含兩行**——① 圖片 `![figX-Y](images/figX-Y.png)`；② 下面獨立一行圖說：**粗體圖號＋全形空格＋圖說完整譯文**（如 `**圖 3-1**　……譯文`）。夾在前後文 cell 間。**圖說譯文在本文有一次、在譯圖裡也有一次，這個重複是刻意的**：看板預設顯示原圖（外文圖說），本文這行是讀者唯一看得到的中文圖說——別當冗餘砍掉。
...
- **怎麼生 → 見你這條路的做法檔**（§5 dispatcher 已指定）：`codex` 在本機用 Python(Pillow) 疊字（照片／結構都像素忠實）；`chatgpt` 預設 image-gen 重繪、少數圖種（照片／勾勾／精確 code／數據曲線）改 PIL。**無論走哪條，核心要求一致**：把圖內文字＋整段圖說翻成目標語言，其餘（圖形／座標軸／數據／曲線／配色／排版）維持原樣；數學符號／變數／產品名不翻。
...
- **🔍 文字專項驗收（每張都做）**：譯圖的價值就是「文字翻對」，**驗的細度依產出方式**：
  ...
  - **疊字（PIL；codex 一律、chatgpt 特例）**：不會 render 錯字型，但**三類風險**——① **版面**（譯文比原文短→殘留原字尾、長→溢出、壓到隔壁、**或工具把字型壓扁／變形硬塞進原空間——內容全對但筆畫糊，要放大才確定不是錯字**）；② **你給的對照本身寫錯**；③ **工具把譯文字串手打錯**（字母顛倒之類；**點名重跑常重複同一個錯**）→ **整張回讀**看得到，但錯字很會滑過去、可疑處逐字對。
...
### 5D. 整章橫向對帳（§5B／§5C 驗單張，這關驗跨圖）
全章圖都產出後，把**重複出現的同一張圖**（路線圖／進度圖／每節開頭的架構圖…）和**跨圖重複的字串**並排比一次：同一句原文的譯法、章號寫法要**統一定稿**，不一致就重生受影響的那幾張。**單張都合格、並排才看得出用詞不一**——這種橫向不一致單張驗收在定義上抓不到、不能被它取代。
...
- ✅ **本文也要「渲染驗收」（和 §5B 驗圖對等，別組完就交）**：圖會逐張回讀，本文同樣不能只組完就算數——要確認在看板上**真的長對**：
  ...
  - **看渲染（CDP）**：開 `http://localhost:5050`，在頁面 context 跑看板的全域函式 `renderIpynb(text, base)` 再驗產出的 DOM（比截圖可靠——文件很長時 headless 截圖深處會全黑）。⚠️ **這步第一個坑：`renderIpynb` 第一參數吃「ipynb 原始 JSON 字串」、不是 parse 過的物件**——傳物件會回『（無法解析的 ipynb）』佔位**且不報錯**，最容易被誤判成「產物壞掉」。⚠️ **渲染器說了算、別照規格書推理批次修**：真有異常就建個「拿渲染器逐個候選試改、只留真讓症狀變少的」迴圈（同 §5B 程式性驗精神），別把「我以為的規則」大範圍套上去、反而改壞本來好的。⚠️ **任何「數符號」的指標（殘留符號、成對符號…）都要先扣掉「本來就該長那樣」的再數**——code／行內 code 裡的原文、跳脫寫的字面符號，都是假陽性；**照假陽性去修＝竄改原文，比漏修更糟**。所以改完要驗「**不該動的沒動**」（同 §5B「只有該改處有差異」）、不只「症狀消失」——歸零≠正確。而**護欄（哪些候選不碰）本身也會誤傷**：條件寫太寬會把待修項偽裝成「無解」（「被擋掉」和「修不掉」長一樣、不留痕跡，比誤改更難發現）——護欄條件要**貼著真正要保護的**寫、還要能**報告自己擋掉了什麼**。最小可跑片段（填 `id`／`ch`；驗筆記把 `kind` 改 `'note'`）：
    ```js
    ...
    async () => {
      ...
      // ↓ 這幾項只是起點——按「這章實際用到的語法」自己補；且**每個「數出來」的都配一個「壞掉」的**（產出存在≠正確：`.katex` 把 `.katex-error` 也算進去了）
      return { katex: d.querySelectorAll('.katex').length, katex錯: d.querySelectorAll('.katex-error').length,  // 配對範例：katex錯應 0
...
- 清掉 `tmp/<書>/pages/`（過程檔；若還要重 crop 可暫留，最後再清）——**可重建的過程檔自己判斷清、清完回報「清了什麼／留了什麼」**，不必逐項問（照 CLAUDE.md 收尾例外）；不可重建、或有跡象別的作業正在用的才留＋說明為什麼留。
