...
**🚀 開多分頁並行、每頁一張（品質＋速度兼顧，實測最佳做法）**：核心原則＝**「一個對話只處理一張圖」**（單張單對話品質最穩、外框線＋外側留白都到位、幾乎不用重裁），但**不要一張做完才做下一張**（太慢）——改成**同時開多個分頁、每頁各一張圖並行跑**，5 張的生成時間幾乎重疊（≈ 單張時間）。
   - **做法**（用 `browser_tabs` 管分頁）：① 開分頁到**一次最多 5 個**（`browser_tabs` action=new）；② 逐頁 select 過去 → `navigate https://chatgpt.com/`（各自全新對話）→ 上傳單張大略圖 → 下裁切指令送出，**不等它跑完就切下一頁送**（把 5 頁全部送出）；③ 全送出後，逐頁 select → 等結果圖出現 → `browser_evaluate` 抓 `alt="輸出圖像"` 的結果 `<img>` → `fetch`→base64 存回 `figX-Y.png`；④ 每頁 §5C 回讀驗收。
   - ✅ **每頁一張的最大好處**：來源↔成品一一對應在各自分頁，**絕不會像批次那樣對錯張**（不必靠 DOM 順序猜）。
   - ⚠️ **一個對話（分頁）只放一張圖**：**別在同一對話裡連續追加上傳下一張**（實測會「顯示卡住／只回下載連結、抓不到內嵌圖」），也**別把多張塞進同一則訊息叫它一次裁**（品質參差、易出細條／漏圖說／切外框線）。要多做就**多開分頁**，不是把量堆進同一頁。
   - ⚠️ **一次最多 5 個分頁**：再多難管理、也吃資源。超過 5 張就**分批**（這批 5 張收完、再開下一批）。
...
- **流程＝接在 §5A 裁切後**，每張圖「**先裁切 → 再生譯圖**」：把剛裁好的 `figX-Y.png` 丟 ChatGPT，指令要它「**只把圖內文字＋圖說翻成目標語言；其餘圖形／座標軸／數據／曲線／數學符號／排版／配色／截圖內容一律維持原樣**，重新輸出整張圖」，附文字對照（含**圖說中文版照抄給它**最準；照 §4a 用語；數學符號與變數、產品名不要翻）。**提示詞要盡量把「除文字翻譯外、其他全部保持原樣」描述清楚。**（純翻圖說的圖：點明「圖體像素不動、只翻最下／旁那行圖說」。第三方截圖照樣會被 GPT 重畫、可能有輕微差異——可接受，原圖才是看板預設。）
- **命名**：存成 **`figX-Y.<語言碼>.png`**，`<語言碼>`＝output 語言資料夾名（**與 §6 同步**）：繁中→`zh_tw`、簡中→`zh_cn`、日文→`ja`…。與原圖**同資料夾**、**絕不覆蓋 `figX-Y.png`**（`figX-Y.png` 永遠是源語言原始裁切）。
- **🚀 開多分頁並行、每頁一張（同 §5A，品質＋速度兼顧）**：核心＝**一個對話只生一張譯圖**（單張單對話品質最穩），但**開多分頁並行**、別逐張等。用 `browser_tabs` 開**一次最多 5 頁**，逐頁 select→`navigate https://chatgpt.com/`（全新對話）→上傳單張原圖＋該圖文字對照→叫 ChatGPT 生繁中版並 `display(Image(...))` 內嵌顯示、送出（**不等它跑完就切下一頁送**）；全部送出後再逐頁 select 收成（抓 `alt="輸出圖像"` 或 `naturalWidth` 較大的生成圖，存 `figX-Y.<語言碼>.png`）。5 張生成時間幾乎重疊、比逐張快很多。超過 5 張就分批。
   - ✅ **每頁一張＝來源↔成品在各自分頁一一對應**，不會對錯張（不必靠 DOM 順序猜、尤其多張同尺寸 plot 時最保險）。
   - ⚠️ **一頁一張**：別在同對話連續追加下一張新圖（會卡住／抓不到圖），也別把多張塞同一則訊息叫它一次生（品質參差、易對錯）。要多做就多開分頁。
   - **修錯要在「同一頁的同對話」內做**：某頁譯圖有字錯，就在**那一頁**點名要它改哪個字、重新輸出（修同一張＝同對話沒問題，實測有效）；這跟「別追加下一張新圖」不衝突（追加的是**別張**才會卡）。**同一張最多 2 次**，2 次還錯留原圖。
