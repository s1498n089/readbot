...
> 🈂️ **「要翻的圖內文字」包含圖說（caption）本身**——「图 X-Y …」/「Figure X-Y …」那行也是外文，也要翻。所以**圖框內沒有可翻文字（純示意圖／節點圖／第三方 UI 截圖）也照樣生譯圖**：圖體（方框／節點／連線／截圖）像素級保持原樣、**只把圖說翻成目標語言**重新輸出。圖說翻譯版很直接、好讀，使用者通常會想要。**唯一可略過的情形**：整張圖連圖說都沒有任何外文（例：無圖說、或圖說已是目標語言）。
- **流程＝接在 §5A 裁切後**，每張圖「**先裁切 → 再生譯圖**」：把剛裁好的 `figX-Y.png` 丟 ChatGPT，要求「**只把圖內文字＋圖說翻成目標語言，其餘（圖形／座標軸／數據／曲線／數學符號／排版／配色）盡量維持原樣**，重新輸出整張圖」。**做法交給 GPT 自行判斷、不要寫死**——實測 GPT 通常會自己選最合適的方式（常自動用 PIL 疊字），圖體維持得很好。**建議範本**（`[…]` 換成該圖實際內容；照 §4a 用語；數學符號與變數、產品名不翻）：
   ...
   - 💡 **想要更保真時**，可加一句「**用 PIL 在原圖上疊字覆蓋、不要重畫**」：實測這樣圖體能像素級零改動、且無缺字問題（code interpreter 字型能出 `x_t`、`ε_θ`、全形標點）。但**非必須**——GPT 自行判斷通常就很好，這只是保底手段。
- **命名**：存成 **`figX-Y.<語言碼>.png`**，`<語言碼>`＝output 語言資料夾名（**與 §6 同步**；task=translate 現行輸出恆為 `zh_tw`）。與原圖**同資料夾**、**絕不覆蓋 `figX-Y.png`**（`figX-Y.png` 永遠是源語言原始裁切）。
- **🚀 並行方式同 §5A——譯圖也絕對是「一個對話一張圖」（呼應 §5 鐵律）**：⛔ **絕不可**把多張圖丟同一個對話叫它一次翻，也**絕不可**在同一對話連續追加下一張——**多張一起翻＝翻錯字／漏字／圖體走樣，效果超差**。要翻多張就**開多分頁、每頁一張、一次最多 5 頁**（超過就分批）：逐頁全新對話→上傳單張原圖＋該圖文字對照→下譯圖指令並要求 `display(Image(...))` 內嵌顯示→全部送出後**輪一圈收成**（用 §5A 步驟 5 的共用收成法：`alt="輸出圖像"` 判完成、`a.download` 攔到 `.browser/out/`、`NOT_READY` 下一輪再回來），`cp` 存成 `figX-Y.<語言碼>.png`。§5D 特有兩點：
   - **收成辨識**：以 `alt="輸出圖像"` 為準（尺寸只當 fallback——譯圖尺寸常與原圖幾乎相同，比大小會誤抓）。
   - **修字錯在「同一頁的同對話」內做**：某頁譯圖驗出字錯，就在**那一頁**點名要它改哪個字、重新輸出——「別追加」限制的是**別張新圖**，修同一張不受限。重生次數上限見下方「文字專項驗收」。
- **trade-off（可接受）**：GPT 若重畫整張圖，數據圖／曲線可能跟原圖對不上（想更保真可用上面的疊字保底句）。無論如何都可接受——看板**預設顯示原裁切圖**（忠實那張），譯圖是使用者**自己點「譯圖」**才看的輔助；真生壞了還有原圖、而且原圖就是預設。
