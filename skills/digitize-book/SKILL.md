...
把頁面插圖截出來。標準流程＝每張圖「**先裁切（成 `figX-Y.png`）→ §5B 回讀驗收 → task=translate 再生譯圖（成 `figX-Y.<語言碼>.png`）**」；原裁切圖一律保留、看板**預設顯示原圖**。裁切要含**整張圖 ＋ 整行圖說**。
...
### 5B. 回讀驗收 ＋ 插回原文
- **逐張回讀驗收**（強制）：每張都自己 Read 回讀，注意力**放在四邊**對照 ①②③④（方框有沒有被切？峰頂在不在？留白有沒有沾到正文／前一張圖？圖說完不完整？）。不符就重來——重跑該圖的裁切（見你這條路的做法檔）。⚠️ **別用「全部乾淨」一句帶過**，每張都要實際看過；圖多可再派 subagent 當獨立第二雙眼複驗。
...
- **看板預設顯示原裁切圖**（忠實那張）；譯圖是使用者**自己點「譯圖」**才看的輔助，真生壞了還有原圖兜底、而且原圖就是預設。修字錯的重生次數上限見下方「文字專項驗收」。
...
- **🔍 文字專項驗收（譯圖最重要的一關，每張都做）**：譯圖的價值就是「文字翻對」，所以要**專門針對文字驗一遍**——不是整張看順眼就過。步驟：
  ...
  - 對不上 → 重生（指令點名要它修哪個字）；**同一張最多 2 次**。**2 次還錯先看「錯多大」再決定收尾、別一律整張作廢**：
