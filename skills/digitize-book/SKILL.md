...
**🚀 並行方式：開多分頁、每頁一張（一次最多 5 頁）**：兩條規則同時成立——**「一個對話只裁一張圖」**（品質最穩）＋**「多張圖開多個分頁同時跑」**（不必等前一張做完，5 張的生成時間幾乎重疊）。
   - **做法**（用 `browser_tabs` 管分頁）：① 開分頁，**一次最多 5 個**（`browser_tabs` action=new）；② 逐頁 select → `navigate https://chatgpt.com/`（各自全新對話）→ 上傳單張大略圖 → 下裁切指令送出，**不等結果就切下一頁送**（把 5 頁全部送出）；③ 全送出後，逐頁 select → 等結果圖出現 → `browser_evaluate` 抓 `alt="輸出圖像"` 的結果 `<img>` → `fetch`→base64 存回 `figX-Y.png`；④ 每頁 §5C 回讀驗收。
...
- **🚀 並行方式同 §5A（開多分頁、每頁一張、一次最多 5 頁）**：逐頁全新對話→上傳單張原圖＋該圖文字對照→要求生成目標語言版並 `display(Image(...))` 內嵌顯示→全部送出後逐頁收成。§5D 特有的兩點：
   - **收成辨識**：生成圖＝`alt="輸出圖像"` 或 **`naturalWidth` 較大**那張（上傳的原圖較小），存 `figX-Y.<語言碼>.png`。
