...
**截圖一律走 CDP→ChatGPT（§5A）**——Claude 的強項是語言，看圖／生圖的多模態交給 ChatGPT。裁切要切到**整張圖 ＋ 整行圖說**，裁完**回讀驗收**（§5C）。**task=translate 時，標準流程＝每張圖「先裁切（§5A）→ 再生譯圖（§5D）」**；原裁切圖一律保留、看板**預設顯示原圖**。
...
> 要一次處理多張，**唯一正確做法＝「開多分頁、每頁一張、一個對話只放一張圖」並行**（裁切見 §5A 並行段、譯圖見 §5D，兩邊一模一樣）。重點是**多開分頁**攤開跑，**絕不是**把量堆進同一個對話。**寧可慢、寧可多開幾批，也不要為了省事把多張塞進一個對話。**
...
5. 等它跑 Python 裁好 → **抓回本機**（**這套收成法 §5A/§5D 共用**）：
...
**🚀 並行方式：開多分頁、每頁一張（一次最多 5 頁）＝唯一正確做法**（呼應 §5 鐵律）：兩條規則同時成立——⛔ **一個對話只裁一張圖**（多張塞同一對話品質必崩：細條／漏圖說／切外框線）＋**多張圖開多個分頁同時跑**（不必等前一張做完，5 張的生成時間幾乎重疊）。
   - **做法**（用 `browser_tabs` 管分頁）：① 開分頁，**一次最多 5 個**（`browser_tabs` action=new）；② 逐頁 select → `navigate https://chatgpt.com/`（各自全新對話）→ 上傳單張大略圖 → 下裁切指令送出，**不等結果就切下一頁送**（把 5 頁全部送出）；③ 全送出後，**輪一圈收成**（依步驟 5 的共用收成法）：逐頁 select → `browser_evaluate` 探 `alt="輸出圖像"`，好的先收（`a.download`→`.browser/out/`→`cp` 成 `figX-Y.png`）、`NOT_READY` 的**下一輪再回來**（別死等某一頁擋住其他頁）；④ 每頁 §5C 回讀驗收。
   ...
   - ⚠️ 備輸入大略圖時**範圍要夠（含整張圖＋下方整行圖說＋周邊一點白，讓 GPT 有空間保住外框線）**；抓太小會出「細條」或「漏圖說」。§5C 驗到就**單獨開一頁重裁**。
...
> 🈂️ **「要翻的圖內文字」包含圖說（caption）本身**——「图 X-Y …」/「Figure X-Y …」那行也是外文，也要翻。所以**圖框內沒有可翻文字（純示意圖／節點圖／第三方 UI 截圖）也照樣生譯圖**：圖體（方框／節點／連線／截圖）維持原樣、不動內容，**只把圖說翻成目標語言**重新輸出（截圖這種不能重畫的走 PIL、見 §5D💡；示意圖走預設 image-gen 即可）。圖說翻譯版很直接、好讀，使用者通常會想要。**唯一可略過的情形**：整張圖連圖說都沒有任何外文（例：無圖說、或圖說已是目標語言）。
...
> 📝 **先讀使用者專案根的 `note-style.md`（若有）→ 照它的口味與切角寫**（§A），並沿用它列的**結構標記**（§B，心智圖靠這套解析）。沒有就用以下預設。
每個小節一塊 `## 1.x <小節標題>`，內含：
- **重點**：2–4 句濃縮。
