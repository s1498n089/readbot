...
**截圖一律走 CDP→ChatGPT（§5A）**——Claude 的強項是語言，看圖／生圖的多模態交給 ChatGPT。裁切要切到**整張圖 ＋ 整行圖說**，裁完**回讀驗收**（§5B）。**task=translate 時，標準流程＝每張圖「先裁切（§5A）→ 再生譯圖（§5C）」**；原裁切圖一律保留、看板**預設顯示原圖**。
...
**合格標準（4 條）**：① **外框邊線完整**（圖本體＋圖外那圈細外框線四邊都不切，尤其曲線**峰頂**）；② **外框線外側再留白邊**（別讓外框線貼在圖片邊緣、四周有呼吸空間，約 20~30px——切到框線一眼就看得出來、很扣分）；③ **留白不含內文**（沒有正文段落、頁眉頁碼、前一張圖）；④ **含整行圖說**（「图 X-Y …」/「Figure X-Y …」；圖說有兩行就兩行都要）。
...
4. 在輸入框打**裁切指令**，按 Enter 送出。**固定用這個通用範本**（把 `图 X-Y` 換成實際編號；知道圖說全文就一起填進去，幫 ChatGPT 定位更準；英文書改用 `Figure X-Y`）——它已把 §5 的 4 條標準寫進去：
   > 這張圖裡有一張圖「**图 X-Y**」。請用 Python（PIL）把「图 X-Y 那張圖，加上它正下方那整行圖說『**图 X-Y …**』」精準裁切成一張 PNG。要求：① 圖外圍那圈細細的**外框邊線要完整保留**、四邊都不可切到它、也不要讓它貼在圖片邊緣；② 在外框線的**外側再多留一點白邊（約 20~30px）**、四周有呼吸空間；③ **不可包含上下或旁邊的內文段落**（正文那幾行不要）；④ **一定要含「图 X-Y …」那整行圖說**（有兩行就兩行都要）。裁好後請用 **IPython.display 的 `display(Image(...))` 把成品直接內嵌顯示**出來（不要只給下載連結）。
   - 例：`X-Y`＝`1-5`、圖說填『均值为0、标准差为1的正态分布』。**外框線＋外側留白這點很重要**——光說「留白邊」GPT 常會切到框線，要明講「外框線外側再留白、別貼邊」。
5. 等它跑 Python 裁好 → **抓回本機**（**這套收成法 §5A/§5C 共用**）：
...
**🚀 並行方式：開多分頁、每頁一張（一次最多 5 頁）＝唯一正確做法**（呼應 §5 鐵律）：兩條規則同時成立——⛔ **一個對話只裁一張圖**（多張塞同一對話品質必崩：細條／漏圖說／切外框線）＋**多張圖開多個分頁同時跑**（不必等前一張做完，5 張的生成時間幾乎重疊）。
   - **做法**（用 `browser_tabs` 管分頁）：① 開分頁，**一次最多 5 個**（`browser_tabs` action=new）；② 逐頁 select → `navigate https://chatgpt.com/`（各自全新對話）→ 上傳單張大略圖 → 下裁切指令送出，**不等結果就切下一頁送**（把 5 頁全部送出）；③ 全送出後，**輪一圈收成**（依步驟 5 的共用收成法）：逐頁 select → `browser_evaluate` 探 `alt="輸出圖像"`，好的先收（`a.download`→`.browser/out/`→`cp` 成 `figX-Y.png`）、`NOT_READY` 的**下一輪再回來**（別死等某一頁擋住其他頁）；④ 每頁 §5B 回讀驗收。
...
- **流程＝接在 §5A 裁切後**，每張圖「**先裁切 → 再生譯圖**」：把剛裁好的 `figX-Y.png` 丟 ChatGPT，要求「**把圖內文字＋圖說翻成目標語言，其餘（圖形／座標軸／數據／曲線／數學符號／排版／配色）維持原樣**，重新輸出整張圖」。**預設做法＝請 GPT 用影像生成「重新繪製整張圖」（image-gen），這是優先選擇**——對結構／示意圖（流程圖、金字塔、漏斗、節點圖、方框圖…）最乾淨、文字換得徹底、不會有殘字問題（GPT 常自己就走這條）。**只有少數圖種要改走 PIL 疊字**（照片／勾勾／精確 code／數據曲線——見下方💡）。**建議範本**（`[…]` 換成該圖實際內容；照 §4a 用語；數學符號與變數、產品名不翻）：
   ...
   - 💡 **改走 PIL 疊字的時機＝image-gen 會「動到不該動的東西」的那幾類圖**（**只有這些才用 PIL，不是預設**）：① **照片／第三方 UI 截圖**（image-gen 會把人臉／內容／截圖整個重畫，真人照變成 AI 生成、截圖被畫走樣，整張圖失去意義）；② **勾勾／標記**（重繪常漏畫）；③ **精確 code／hex／數字**（重繪可能改字）；④ **數據圖／曲線**（重繪對不上原數據）。這幾類才明講「**務必用 code interpreter＋PIL 在原圖上疊字覆蓋、絕對不要用影像生成重畫**」，並**把理由一起講**（GPT 才乖乖走 PIL），PIL 能像素級零改動、也不缺字（`x_t`、`ε_θ`、全形標點都出得來）。⚠️ **PIL 疊字自己的毛病**：遮蓋色塊常只按「譯文寬度」開，**譯文比原文短時右緣會殘留原文字尾**（例：Problem-Solver 只蓋一半、露出 `er`）→ 同對話請它「**把每條的遮蓋色塊加寬到蓋掉整段原文、再重新輸出**」。
...
整章一檔（跟本文一樣 ipynb）→ `…/ipynb/note/ch<n>/ch<n>.ipynb`。
> 📝 **先讀使用者專案根的 `note-style.md`（若有）→ 照它寫**：口味、切角、結構標記全以它為準（使用者自訂的個人偏好、**讀到什麼照什麼，skill 不預設它列了什麼**）。沒有就用以下預設。
每個小節一塊 `## 1.x <小節標題>`，內含：
- **重點**：2–4 句濃縮。
- **白話打痛點**：站在**初學者（小白）**的角度——這節真正想解決什麼問題？一般讀者最容易**卡在哪、誤解什麼**？用**具體例子**、或**走一遍實際流程／一個小數字**把它講通。很多書寫得抽象、對小白不友善；這裡就是要把它「翻成人話」。
- **關鍵詞**：幾個術語。
- **程式輔助教學**：若該節有數學式或程式，note 可放**一小段可跑的 code cell** 把概念演示出來（畫個分布圖、跑個最小例子、用程式驗證一條公式…），用「能跑的程式」幫讀者建立直覺，比純文字更好懂。（code cell **不附預先算好的 outputs**；看板只顯示程式碼、不會執行，讀者在 Jupyter 跑才出圖。）
