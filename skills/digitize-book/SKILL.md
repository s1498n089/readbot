...
- ✅ **本文也要「渲染驗收」（和 §5B 驗圖對等，別組完就交）**：圖會逐張回讀，本文同樣不能只組完就算數——要確認在看板上**真的長對**：
  ...
  - **看渲染（CDP）**：開 `http://localhost:5050`，在頁面 context 跑看板的全域函式 `renderIpynb(text, base)` 再驗產出的 DOM（比截圖可靠——文件很長時 headless 截圖深處會全黑）。⚠️ **渲染器說了算、別照規格書推理批次修**：真有異常就建個「拿渲染器逐個候選試改、只留真讓症狀變少的」迴圈（同 §5B 程式性驗精神），別把「我以為的規則」大範圍套上去、反而改壞本來好的。⚠️ **任何「數符號」的指標（殘留符號、成對符號…）都要先扣掉「本來就該長那樣」的再數**——code／行內 code 裡的原文、跳脫寫的字面符號，都是假陽性；**照假陽性去修＝竄改原文，比漏修更糟**。所以改完要驗「**不該動的沒動**」（同 §5B「只有該改處有差異」）、不只「症狀消失」——歸零≠正確。⚠️ **第一參數吃「ipynb 原始 JSON 字串」、不是 parse 過的物件**——傳物件會回『（無法解析的 ipynb）』佔位**且不報錯**，最容易被誤判成產物壞掉。最小可跑片段（填 `id`／`ch`；驗筆記把 `kind` 改 `'note'`）：
    ```js
    ...
    async () => {
      ...
      // ↓ 這幾項只是起點——按「這章實際用到的語法」自己補（別把它當完整清單）
