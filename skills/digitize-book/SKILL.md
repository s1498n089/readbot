...
- **讀 `book/<書>/config.json`** → 拿設定當預設，不必每次重問使用者（檔案不存在＝全部視同預設值，可順手建立；schema 見 README「資料模型」）：
  ...
  - **`image_model`**：`codex`（預設，本機 Python／PIL 直接裁切／疊字）或 `chatgpt`（CDP 接管 ChatGPT）——**決定 §5 截圖走哪條做法檔、開工前要讀**。⚠️ `codex` 需本機有 codex＋登入——**實際自檢綁在做法檔「真跑前」**（中段接手／續作不一定走到這，見 figure-codex 前提）；缺就停、請改 `chatgpt` 或裝好。`mode` 仍是**保留欄位**（目前一律 `ipynb`）。
...
- **🔍 文字專項驗收（每張都做）**：譯圖的價值就是「文字翻對」，**驗的細度依產出方式**：
  ...
  - **疊字（PIL；codex 一律、chatgpt 特例）**：程式性畫字、不會畫錯字，風險在**版面**（譯文比原文短→殘留原字尾、比原文長→溢出畫布或壓到隔壁）＋**你給的對照本身寫錯** → **整張回讀**就看得到，可疑處再放大。
...
- ✅ **本文也要「渲染驗收」（和 §5B 驗圖對等，別組完就交）**：圖會逐張回讀，本文同樣不能只組完就算數——要確認在看板上**真的長對**：
  - **靜態掃**：**盤點這章實際用到的語法逐一確認**，別只照名單掃 `[^` 註腳——粗體／斜體／表格／圍欄都可能壞，**CJK 與標點交界處尤其危險**（多數渲染器的強調語法對 CJK 不友善，「粗體結尾緊接中文」常整段失效）；並查**組裝把跨行結構切壞沒**（圍欄／表格被切開就毀，可程式性查成對符號是否落在同一 cell，同 §5B 精神）。
  - **看渲染（CDP）**：開 `http://localhost:5050`，在頁面 context 跑看板的全域函式 `renderIpynb(text, base)` 再驗產出的 DOM（比截圖可靠——文件很長時 headless 截圖深處會全黑）。⚠️ **渲染器說了算、別照規格書推理批次修**：真有異常就建個「拿渲染器逐個候選試改、只留真讓症狀變少的」迴圈（同 §5B 程式性驗精神），別把「我以為的規則」大範圍套上去、反而改壞本來好的。⚠️ **第一參數吃「ipynb 原始 JSON 字串」、不是 parse 過的物件**——傳物件會回『（無法解析的 ipynb）』佔位**且不報錯**，最容易被誤判成產物壞掉。最小可跑片段（填 `id`／`ch`；驗筆記把 `kind` 改 `'note'`）：
    ```js
...
- **長章節用「逐節寫工作檔 → 最後組裝」**：每做完一個小節就把該節產出（含 ```python 圍欄、`$$` 數學、`![](images/..)` 圖）寫成 `tmp/<書>/work/<節>.md`；全章做完再用一支小腳本把這些 md 依序組成 ipynb（```python 圍欄→code cell、其餘→markdown cell，依標題切 cell——**但標題判定要略過圍欄與 blockquote 內的行**，引用進來的內容常自帶標題符號、誤切會把一個圍欄劈成兩個 cell）。這樣進度落地、context 被摘要也不怕，且 md→ipynb 的切割邏輯只寫一次。
