...
**🚀 並行方式：開多分頁、每頁一張（一次最多 5 頁）**：兩條規則同時成立——**「一個對話只裁一張圖」**（品質最穩）＋**「多張圖開多個分頁同時跑」**（不必等前一張做完，5 張的生成時間幾乎重疊）。
...
- **🚀 並行方式同 §5A（開多分頁、每頁一張、一次最多 5 頁）**：逐頁全新對話→上傳單張原圖＋該圖文字對照→下譯圖指令並要求 `display(Image(...))` 內嵌顯示→全部送出後**輪一圈收成**（用 §5A 步驟 5 的共用收成法：`alt="輸出圖像"` 判完成、`a.download` 攔到 `.browser/out/`、`NOT_READY` 下一輪再回來），`cp` 存成 `figX-Y.<語言碼>.png`。§5D 特有兩點：
