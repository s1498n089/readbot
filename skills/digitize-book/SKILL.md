...
- **🔍 文字專項驗收（譯圖最重要的一關，每張都做）**：譯圖的價值就是「文字翻對」，所以要**專門針對文字驗一遍**——不是整張看順眼就過。步驟：
  ...
  - 對不上 → 重生（指令點名要它修哪個字）；**同一張最多 2 次**。**2 次還錯先看「錯多大」再決定收尾、別一律整張作廢**：
    ...
    - **只有一兩個字錯、其餘全對**（版面／配色／其他文字都對）→ **本機 PIL 只補那一兩個字**是合理的最後收尾（image-gen 兩次都救不回才用、非常態）：定位到錯字位置 → 白色蓋掉 → 用**接近書體的字型**重寫正確的字（原文是斜體就仿斜體）、其餘像素完全不動。（典型情境：圖說某個字被 image-gen 寫成形近的錯字、點名重生兩次都改不掉，但整張只錯這一字——補那一字，勝過整張作廢。）
