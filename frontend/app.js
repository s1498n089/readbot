...
createApp({
  data() {
    return {
      ...
      imageModels: [{ v: 'codex', label: 'Codex' }, { v: 'chatgpt', label: 'ChatGPT' }],
  ...
  methods: {
    ...
    async openDoc(c) {
      ...
      try {
        ...
        const [figData, r] = await Promise.all([
          fetch('/api/books/' + id + '/figs?' + q.toString()).then((x) => (x.ok ? x.json() : { translated: [] })).catch(() => ({ translated: [] })),
    ...
    // ----- 每本書設定（task 任務、image_model 截圖做法；mode 仍為保留欄位）→ PUT 寫進 config.json -----
